...
before the number on the first string. A is Uppercase and its position in the alphabet is 1. So we divide the number
12 with the position 1 (12/1 = 12). Then we move to the letter after the number. b is lowercase and its position is 2.
So we add 2 to the resulted number (12+2=14). Similarly for the second string s is lowercase and its position is 19 so
we multiply it with the number (17*19 = 323). Then we have Uppercase G with position 7, so we subtract it from the
...
more white spaces. The input data will always be valid and in the format described. There is no need to check it explicitly.
Output
...
public class Ex8_LettersChangeNumbers {
    ...
    public static double calculate(String current) {
        char first = current.charAt(0);
        char second = current.charAt(current.length() - 1);
        String stringNumber = "";
        stringNumber = current.substring(1, current.length() - 1);

        int positionInAlphabetic = 0;
        double number = Double.parseDouble(stringNumber);
        double sum = 0;

        if (Character.isUpperCase(first)) {
            positionInAlphabetic = first - 64;
            sum += (number / positionInAlphabetic);
        } else {
            positionInAlphabetic = first - 96;
            sum += (number * positionInAlphabetic);
        }

        if (Character.isUpperCase(second)) {
            positionInAlphabetic = second - 64;
            sum -= positionInAlphabetic;
        } else {
            positionInAlphabetic = second - 96;
            sum += positionInAlphabetic;
        }
        return sum;
    }
