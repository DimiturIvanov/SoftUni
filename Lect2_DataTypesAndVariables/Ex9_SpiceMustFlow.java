...
Arrakis, under constant threat of giant sand worms. To make the work as efficient as possible, the Duke has tasked
you with the creation of a management software.
...
public class Ex9_SpiceMustFlow {
    public static void main(String[] args) {
        ...
        int crewConsumes = 26;
        int yieldDrops = 10;
        int sumAllCrewConsumes = 0;
        int day = 0;

        while (startingYield >= 100) {
            sumAllCrewConsumes += startingYield - crewConsumes;
            startingYield -= yieldDrops;
            day++;

        }

        if (sumAllCrewConsumes >= crewConsumes) {
            sumAllCrewConsumes -= crewConsumes;
        }

        System.out.println(day);
        System.out.println(sumAllCrewConsumes);
